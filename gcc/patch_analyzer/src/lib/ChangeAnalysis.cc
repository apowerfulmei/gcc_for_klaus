...
GenSetPair ChangeAnalysisPass::analyzeValueSetRange(BasicBlockPair pair,
  ...
  std::set<BasicBlock *> visitedBasicBlock;
  while (!changeSet.empty()) {
    BasicBlock *current = changeSet.front();
    changeSet.pop_front();
    if (visitedBasicBlock.find(current) != visitedBasicBlock.end()) {
      continue;
    }
    visitedBasicBlock.insert(current);
    changed = false;
    condChanged = false;
    ...
    if (current != start) {
      ...
      changed |= unionSet(blockExit[current].second, blockEntry[current].second);
...
std::set<Value *> ChangeAnalysisPass::findVariables(Function *func) {
  ...
  for (auto &inst : instructions(*func)) {
    ...
    if (auto *dbgV = dyn_cast<DbgValueInst>(I)) {
      ...
      dbgVars.insert(dbgV->getValue());
      // all debug variables are interesting.
      resVars.insert(dbgV->getValue());

      // store the debug information
      if (DbgMap.find(dbgV->getValue()) != DbgMap.end()) {
        KA_LOGS(1, "old " << *DbgMap[dbgV->getValue()]);
        KA_LOGS(1, "new " << *dbgV);
        // assert(DbgMap[dbgV->getVariableLocation()] == dbgV);
      }
      DbgMap[dbgV->getValue()] = cast<Value>(dbgV);
